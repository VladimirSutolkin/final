...
<div th:each="user,rowStat : ${users}"
    ...
    <a type="button" th:href="@{/users/{id}/edit(id=${user.getId()})}"
       th:classappend="${rowStat.count % 2 == 0 ? 'bg-light text-dark' : 'bg-secondary text-white'}">Edit</a>
    <a type="button" th:href="@{/users/delete(id=${user.getId()})}" class="btn btn-secondary"
       th:classappend="${rowStat.count % 2 == 0 ? 'bg-light text-dark' : 'bg-secondary text-white'}">Delete</a>
    <p>
    <p>
